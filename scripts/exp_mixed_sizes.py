"""Experiment: Mixed Sizes — Pure Vision Zone Identification with varied object sizes.

Gemma must identify objects of different sizes (small, medium, large)
and report the correct size category.

Usage:
    python scripts/exp_mixed_sizes.py --runs 150 --output overnight_results/mixed_sizes/r6_mixed_sizes.jsonl --variation mixed_sizes --no-grid --temperature 0.0
"""
...
# Radii to sample from (small -> medium -> large)
        ...
        "properties": {
            "observed_objects": {
                ...
                "items": {
                    ...
                    "required": ["description", "color", "zone", "shape", "size"],
...
Identify every object you can see. For each, describe its color, shape, size category, and which zone it occupies.
Use the size categories: small, medium, or large — based on how big the object appears in the image.
...
def evaluate(parsed: dict, ground_truth: list[dict]) -> dict:
    ...
    for oi, obs in enumerate(observed):
        ...
        if best_gi is not None:
            ...
            # Size accuracy — only if model reported a size
            if obs_size:
                size_total += 1
...
def main():
    ...
    parser.add_argument("--variation", type=str, default="mixed_sizes",
    ...
    for si, layout in enumerate(SCENE_LAYOUTS):
        ...
        for rep in range(reps):
            ...
            prompt = (
                ...
                "Identify every object you can see by its color, shape, size (small, medium, or large), "
                "and which zone it occupies. Be specific about what zone each object is in."
            )
